...
/**
 * Synchronizes blockstate files
 */
class BlockstateSynchronizer : Synchronizer {
    ...
    override fun sync(
        ...
        Logger.info("Synchronizing blockstates from $source to $target...")

        val sourceDir = resolveBlockstatesDir(projectDir, source)
        val targetDir = resolveBlockstatesDir(projectDir, target)

        if (!sourceDir.exists()) {
            return SyncResult(
                success = false,
                message = "Source blockstates directory not found: ${sourceDir.absolutePath}"
            )
        }
        ...
        return SyncResult(
            ...
            conflicts = conflicts,
            success = true,
            message = "Blockstate sync completed: ${copied.size} copied, ${skipped.size} skipped"
        )
    }

    private fun resolveBlockstatesDir(projectDir: File, path: String): File {
        ...
        if (versionDir.exists()) {
            ...
            if (assetsDir != null) {
                ...
                if (modIdDir != null) {
                    return File(modIdDir, "blockstates")
                }
            }
        }
    ...
    private fun createDryRunResult(diff: DiffResult, sourceDir: File, targetDir: File): SyncResult {
        ...
        return SyncResult(
            ...
            conflicts = conflicts,
            success = true,
            message = "Dry run: ${changes.size} blockstates would be synced, ${conflicts.size} conflicts"
        )
    }
}
