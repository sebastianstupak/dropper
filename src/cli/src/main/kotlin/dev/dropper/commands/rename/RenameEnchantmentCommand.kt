 ...
 */
class RenameEnchantmentCommand : DropperCommand(
    name = "enchantment",
    ...
    override fun run() {
        ...
        val context = RenameContext(
            ...
            oldName = oldName,
            newName = newName,
            componentType = ComponentType.ENCHANTMENT,
            version = version,
            dryRun = dryRun,
            force = force
        )

        val renamer = EnchantmentRenamer()
        ...
        if (discoveredFiles.isEmpty()) {
            Logger.error("Enchantment '$oldName' not found")
            return
        }
        ...
        if (result.success) {
            if (dryRun) {
                ...
                Logger.success("Successfully renamed enchantment: $oldName -> $newName")
