...
class RemoveEnchantmentCommand : DropperCommand(
    ...
    private val name by argument(help = "Enchantment name in snake_case")
    ...
    override fun run() {
        ...
        if (!force && !dryRun) {
            Logger.warn("This will remove enchantment '$name' and all associated files")
            print("Continue? (y/n): ")
        ...
        val result = EnchantmentRemover().remove(projectDir, name, modId, options)

        if (result.success) {
            Logger.success(if (dryRun) "DRY RUN: Would remove ${result.filesRemoved.size} file(s)"
                          else "Successfully removed enchantment '$name'")
        } else {
