...
/**
 * Launch Minecraft server only
 */
class DevServerCommand : CliktCommand(
    name = "server",
    help = "Launch Minecraft server in development mode"
) {
    ...
    override fun run() {
        ...
        Logger.info("Starting Minecraft $selectedVersion server with $selectedLoader loader...")
        Logger.info("Server will start on port 25565")
        val versionGradle = configReader.versionToGradleFormat(selectedVersion)
        Logger.info("Executing: ./gradlew :${versionGradle}-${selectedLoader}:runServer")
        println()
        ...
        println()
        if (exitCode == 0) {
            Logger.success("Minecraft server started successfully!")
        } else {
            Logger.error("Failed to start Minecraft server (exit code $exitCode)")
        }
    }
}
