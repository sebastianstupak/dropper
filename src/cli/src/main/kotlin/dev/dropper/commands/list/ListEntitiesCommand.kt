...
class ListEntitiesCommand : CliktCommand(
    ...
    override fun run() {
        ...
        var entities = cache?.get("entities") ?: EntityIndexer().index(projectDir)

        if (cache == null) {
            val components = mapOf("entities" to entities)
            IndexCache.save(projectDir, components)
        }

        // Apply filters
        if (search != null) {
            entities = entities.filter { it.name.contains(search!!, ignoreCase = true) }
        }

        if (version != null) {
            entities = entities.filter { it.versions.contains(version) }
        }

        if (loader != null) {
            entities = entities.filter { it.loaders.contains(loader) }
        }

        // Format output
        val formatter = FormatterFactory.getFormatter(format)
        val output = formatter.format(entities, "entity")
