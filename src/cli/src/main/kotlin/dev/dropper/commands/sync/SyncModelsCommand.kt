...
import dev.dropper.synchronizers.ModelSynchronizer
import dev.dropper.synchronizers.SyncOptions
import dev.dropper.util.Logger
import java.io.File

/**
 * Command to sync model files only
 */
class SyncModelsCommand : CliktCommand(
    name = "models",
    help = "Sync model files only"
) {
    ...
    override fun run() {
        ...
        Logger.info("Syncing models: $from -> $to")
        if (dryRun) {
        ...
        if (result.copied.isNotEmpty()) {
            Logger.info("\n${if (dryRun) "Would copy" else "Copied"} ${result.copied.size} model(s):")
        ...
        if (result.skipped.isNotEmpty()) {
            Logger.info("\n${if (dryRun) "Would skip" else "Skipped"} ${result.skipped.size} model(s)")
        }
