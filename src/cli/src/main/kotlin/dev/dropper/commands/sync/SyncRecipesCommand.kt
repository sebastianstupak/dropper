...
import dev.dropper.synchronizers.DataSynchronizer
import dev.dropper.synchronizers.SyncOptions
import dev.dropper.util.Logger
import java.io.File

/**
 * Command to sync data files (recipes, loot tables, tags, etc.)
 */
class SyncRecipesCommand : CliktCommand(
    name = "recipes",
    help = "Sync data files (recipes, loot tables, tags, etc.)"
) {
    ...
    override fun run() {
        ...
        Logger.info("Syncing data files: $from -> $to")
        if (dryRun) {
        ...
        val synchronizer = DataSynchronizer()
        ...
        if (result.skipped.isNotEmpty()) {
            Logger.info("\n${if (dryRun) "Would skip" else "Skipped"} ${result.skipped.size} file(s)")
        }
