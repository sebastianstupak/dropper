...
import dev.dropper.indexer.BlockIndexer
 ...
 */
class ListBlocksCommand : DropperCommand(
    name = "blocks",
    ...
    override fun run() {
        ...
        var blocks = cache?.get("blocks") ?: BlockIndexer().index(projectDir)

        if (cache == null) {
            val components = mapOf("blocks" to blocks)
            IndexCache.save(projectDir, components)
        }

        // Apply filters
        if (search != null) {
            blocks = blocks.filter { it.name.contains(search!!, ignoreCase = true) }
        }

        if (version != null) {
            blocks = blocks.filter { it.versions.contains(version) }
        }

        if (loader != null) {
            blocks = blocks.filter { it.loaders.contains(loader) }
        }

        // Format output
        val formatter = FormatterFactory.getFormatter(format)
        val output = formatter.format(blocks, "block")
