...
/**
 * Launch Minecraft client only
 */
class DevClientCommand : CliktCommand(
    name = "client",
    help = "Launch Minecraft client in development mode"
) {
    ...
    override fun run() {
        ...
        Logger.info("Starting Minecraft $selectedVersion client with $selectedLoader loader...")
        val versionGradle = configReader.versionToGradleFormat(selectedVersion)
        Logger.info("Executing: ./gradlew :${versionGradle}-${selectedLoader}:runClient")
        println()
        ...
        println()
        if (exitCode == 0) {
            Logger.success("Minecraft client started successfully!")
        } else {
            Logger.error("Failed to start Minecraft client (exit code $exitCode)")
        }
    }
}
