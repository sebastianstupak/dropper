...
import dev.dropper.removers.BiomeRemover
import dev.dropper.removers.RemovalOptions
import dev.dropper.util.Logger

class RemoveBiomeCommand : DropperCommand(
    name = "biome",
    help = "Remove a biome"
) {
    private val name by argument(help = "Biome name in snake_case")
    ...
    override fun run() {
        ...
        if (!force && !dryRun) {
            Logger.warn("This will remove biome '$name'")
            print("Continue? (y/n): ")
        ...
        val result = BiomeRemover().remove(projectDir, name, modId, options)

        if (result.success) {
            Logger.success(if (dryRun) "DRY RUN: Would remove ${result.filesRemoved.size} file(s)"
                          else "Successfully removed biome '$name'")
        } else {
