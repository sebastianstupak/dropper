...
import dev.dropper.synchronizers.LangSynchronizer
import dev.dropper.synchronizers.SyncOptions
import dev.dropper.util.Logger
import java.io.File

/**
 * Command to sync language files with intelligent merging
 */
class SyncLangCommand : CliktCommand(
    name = "lang",
    help = "Sync language files (translations)"
) {
    ...
    override fun run() {
        ...
        Logger.info("Syncing lang files: $from -> $to")
        if (dryRun) {
        ...
        val synchronizer = LangSynchronizer()
        ...
        if (result.copied.isNotEmpty()) {
            Logger.info("\n${if (dryRun) "Would copy" else "Copied"} ${result.copied.size} new lang file(s)")
        }

        if (result.merged.isNotEmpty()) {
            Logger.info("\n${if (dryRun) "Would merge" else "Merged"} ${result.merged.size} lang file(s):")
