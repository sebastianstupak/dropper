...
import dev.dropper.synchronizers.TextureSynchronizer
import dev.dropper.util.Logger
import java.io.File

/**
 * Command to sync texture files only
 */
class SyncTexturesCommand : CliktCommand(
    name = "textures",
    help = "Sync texture files only"
) {
    ...
    override fun run() {
        ...
        Logger.info("Syncing textures: $from -> $to")
        if (dryRun) {
        ...
        val synchronizer = TextureSynchronizer()
        ...
        if (result.copied.isNotEmpty()) {
            Logger.info("\n${if (dryRun) "Would copy" else "Copied"} ${result.copied.size} texture(s):")
        ...
        if (result.skipped.isNotEmpty()) {
            Logger.info("\n${if (dryRun) "Would skip" else "Skipped"} ${result.skipped.size} texture(s)")
        }
