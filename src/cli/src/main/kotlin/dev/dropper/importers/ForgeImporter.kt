...
/**
 * Imports Forge mods into Dropper structure
 */
class ForgeImporter(
    ...
    override fun import(source: File, target: File) {
        Logger.info("Importing Forge mod from: ${source.absolutePath}")
        ...
        Logger.success("Successfully imported Forge mod!")
    }
    ...
    override fun analyze(source: File): ImportResult {
        ...
        val metadataFile = analyzer.findMetadataFile(source, "forge")
            ?: throw IllegalArgumentException("META-INF/mods.toml not found")

        val metadata = parseForgeMetadata(metadataFile)
    ...
    private fun parseForgeMetadata(file: File): Map<String, String> {
    ...
    private fun mapSourceFiles(source: File, target: File, result: ImportResult) {
        ...
        val sourceFiles = analyzer.findSourceFiles(source)
        val categorized = mapper.categorizeSourceFiles(sourceFiles, "forge")
        ...
        categorized.loaderSpecific.forEach { file ->
            val relativePath = file.toRelativeString(srcDir)
            val targetFile = File(target, "shared/forge/src/main/java/$relativePath")
            FileUtil.writeText(targetFile, file.readText())
        }
    }
