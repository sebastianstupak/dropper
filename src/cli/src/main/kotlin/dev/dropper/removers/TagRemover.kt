 ...
 * Removes tags
 */
class TagRemover : ComponentRemover {
    ...
    override fun findRelatedFiles(projectDir: File, componentName: String, modId: String): List<File> {
        ...
        tagsDirs.forEach { tagsDir ->
            val tagFile = File(tagsDir, "$componentName.json")
            if (tagFile.exists()) {
                files.add(tagFile)
            }
        }
    ...
    override fun findDependencies(projectDir: File, componentName: String, modId: String): List<Dependency> {
        // Tags typically don't have dependencies
        return emptyList()
    }
    ...
    override fun remove(
        ...
        if (relatedFiles.isEmpty()) {
            return RemovalResult(
                success = false,
                errors = listOf("Tag '$componentName' not found")
            )
        }
    ...
    private fun createBackup(projectDir: File, componentName: String, files: List<File>) {
        val backupDir = File(projectDir, ".dropper/backups/${System.currentTimeMillis()}_tag_$componentName")
        backupDir.mkdirs()
