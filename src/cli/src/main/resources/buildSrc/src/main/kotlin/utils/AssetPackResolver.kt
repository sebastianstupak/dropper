...
class AssetPackResolver(private val rootDir: File) {
     ...
     * Returns src/main/java directories following Maven/Gradle standard structure.
     */
    fun resolveSourceDirs(version: String, loader: String): List<File> {
        ...
        // Layer 1: shared/common/src/main/java
        val sharedCommon = File(rootDir, "shared/common/src/main/java")
        if (sharedCommon.exists()) {
            dirs.add(sharedCommon)
        }

        // Layer 2: shared/{loader}/src/main/java
        val sharedLoader = File(rootDir, "shared/$loader/src/main/java")
        ...
        chain.forEach { pack ->
            val packCommon = File(rootDir, "versions/shared/$pack/common/src/main/java")
        ...
        val versionCommon = File(rootDir, "versions/$version/common/src/main/java")
        if (versionCommon.exists()) {
            dirs.add(versionCommon)
        }

        // Layer 5: version+loader specific
        val versionLoader = File(rootDir, "versions/$version/$loader/src/main/java")
