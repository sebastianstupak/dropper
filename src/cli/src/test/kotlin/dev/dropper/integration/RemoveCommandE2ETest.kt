...
class RemoveCommandE2ETest {
    ...
    fun setup() {
        ...
        testProjectDir = File("build/test-remove/${System.currentTimeMillis()}/test-mod")
    ...
    fun cleanup() {
        ...
        if (testProjectDir.exists()) {
            testProjectDir.parentFile?.deleteRecursively()
        }
    }
    ...
    fun `remove item deletes all related files`() {
        ...
        val itemFile = File(testProjectDir, "shared/common/src/main/java/com/testremove/items/RubyGem.java")
        assertTrue(itemFile.exists(), "Item should exist before removal")

        // Remove the item with force flag
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "ruby_gem", "testremove", RemovalOptions(force = true))
        ...
        assertFalse(File(testProjectDir, "shared/fabric/src/main/java/com/testremove/platform/fabric/RubyGemFabric.java").exists())
        assertFalse(File(testProjectDir, "shared/forge/src/main/java/com/testremove/platform/forge/RubyGemForge.java").exists())
        assertFalse(File(testProjectDir, "shared/neoforge/src/main/java/com/testremove/platform/neoforge/RubyGemNeoForge.java").exists())
        assertFalse(File(testProjectDir, "versions/shared/v1/assets/testremove/models/item/ruby_gem.json").exists())
        assertFalse(File(testProjectDir, "versions/shared/v1/assets/testremove/textures/item/ruby_gem.png").exists())
    ...
    fun `remove block deletes all related files including variants`() {
        ...
        val blockFile = File(testProjectDir, "shared/common/src/main/java/com/testremove/blocks/RubySlab.java")
        assertTrue(blockFile.exists())

        // Remove with force
        val remover = BlockRemover()
        val result = remover.remove(testProjectDir, "ruby_slab", "testremove", RemovalOptions(force = true))

        assertTrue(result.success)
        assertFalse(blockFile.exists())

        // Verify blockstate removed
        assertFalse(File(testProjectDir, "versions/shared/v1/assets/testremove/blockstates/ruby_slab.json").exists())

        // Verify all slab variant models removed
        assertFalse(File(testProjectDir, "versions/shared/v1/assets/testremove/models/block/ruby_slab.json").exists())
        assertFalse(File(testProjectDir, "versions/shared/v1/assets/testremove/models/block/ruby_slab_top.json").exists())
        assertFalse(File(testProjectDir, "versions/shared/v1/assets/testremove/models/block/ruby_slab_double.json").exists())

        // Verify item model removed
        assertFalse(File(testProjectDir, "versions/shared/v1/assets/testremove/models/item/ruby_slab.json").exists())

        // Verify loot table removed
        assertFalse(File(testProjectDir, "versions/shared/v1/data/testremove/loot_table/blocks/ruby_slab.json").exists())
    ...
    fun `remove recipe deletes recipe file`() {
        ...
        val recipeFile = File(testProjectDir, "versions/shared/v1/data/testremove/recipe/diamond_sword.json")
        assertTrue(recipeFile.exists())

        // Remove just the recipe
        val remover = RecipeRemover()
        val result = remover.remove(testProjectDir, "diamond_sword", "testremove", RemovalOptions(force = true))
    ...
    fun `remove tag deletes tag file`() {
        ...
        val tagFile = File(testProjectDir, "versions/shared/v1/data/testremove/tags/items/test_tag.json")
        ...
        val result = remover.remove(testProjectDir, "test_tag", "testremove", RemovalOptions(force = true))
    ...
    fun `remove entity deletes all entity files`() {
        ...
        val entityFile = File(testProjectDir, "shared/common/src/main/java/com/testremove/entities/TestEntity.java")
        FileUtil.writeText(entityFile, "public class TestEntity {}")

        val fabricFile = File(testProjectDir, "shared/fabric/src/main/java/com/testremove/platform/fabric/TestEntityFabric.java")
        ...
        val result = remover.remove(testProjectDir, "test_entity", "testremove", RemovalOptions(force = true))
    ...
    fun `remove enchantment deletes all enchantment files`() {
        ...
        val enchFile = File(testProjectDir, "shared/common/src/main/java/com/testremove/enchantments/TestEnchantment.java")
        ...
        val result = remover.remove(testProjectDir, "test_enchantment", "testremove", RemovalOptions(force = true))
    ...
    fun `remove biome deletes biome file`() {
        ...
        val biomeFile = File(testProjectDir, "versions/shared/v1/data/testremove/worldgen/biome/test_biome.json")
        ...
        val result = remover.remove(testProjectDir, "test_biome", "testremove", RemovalOptions(force = true))
    ...
    fun `dry-run shows files to delete without deleting`() {
        ...
        val itemFile = File(testProjectDir, "shared/common/src/main/java/com/testremove/items/TestItem.java")
        assertTrue(itemFile.exists())

        // Dry-run removal
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "test_item", "testremove", RemovalOptions(dryRun = true))
    ...
    fun `dry-run shows accurate file count`() {
        ...
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "preview_item", "testremove", RemovalOptions(dryRun = true))
    ...
    fun `removal blocked when recipe uses item`() {
        ...
        val recipeFile = File(testProjectDir, "versions/shared/v1/data/testremove/recipe/consumer_recipe.json")
        ...
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "used_item", "testremove", RemovalOptions(force = false))
    ...
    fun `removal proceeds with force when dependencies exist`() {
        ...
        val recipeFile = File(testProjectDir, "versions/shared/v1/data/testremove/recipe/using_force_item.json")
        FileUtil.writeText(recipeFile, """
            {
        ...
        // Remove with force
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "force_item", "testremove", RemovalOptions(force = true))
    ...
    fun `warn when tag references block`() {
        ...
        val tagFile = File(testProjectDir, "versions/shared/v1/data/testremove/tags/blocks/stone_blocks.json")
        ...
        // Try to remove without force
        val remover = BlockRemover()
        val result = remover.remove(testProjectDir, "tagged_block", "testremove", RemovalOptions(force = false))
    ...
    fun `detect loot table dependencies`() {
        ...
        val lootFile = File(testProjectDir, "versions/shared/v1/data/testremove/loot_table/chests/custom_chest.json")
        ...
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "loot_item", "testremove", RemovalOptions(force = false))
    ...
    fun `keep-assets removes code but preserves textures`() {
        ...
        val itemFile = File(testProjectDir, "shared/common/src/main/java/com/testremove/items/AssetItem.java")
        val textureFile = File(testProjectDir, "versions/shared/v1/assets/testremove/textures/item/asset_item.png")
        ...
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "asset_item", "testremove", RemovalOptions(force = true, keepAssets = true))
    ...
    fun `empty directories are removed after deletion`() {
        ...
        val itemFile = File(testProjectDir, "shared/common/src/main/java/com/testremove/items/CleanupItem.java")
        ...
        // Remove item
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "cleanup_item", "testremove", RemovalOptions(force = true))
    ...
    fun `removing non-existent component fails gracefully`() {
        ...
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "nonexistent_item", "testremove", RemovalOptions(force = true))
    ...
    fun `removing already deleted component fails`() {
        ...
        // First deletion
        val result1 = remover.remove(testProjectDir, "double_delete", "testremove", RemovalOptions(force = true))
        assertTrue(result1.success)

        // Second deletion (already removed)
        val result2 = remover.remove(testProjectDir, "double_delete", "testremove", RemovalOptions(force = true))
    ...
    fun `remove block with crop stages deletes all stage models`() {
        ...
        for (age in 0..7) {
            val stageModel = File(testProjectDir, "versions/shared/v1/assets/testremove/models/block/test_crop_stage$age.json")
            assertTrue(stageModel.exists(), "Stage $age model should exist")
        }

        // Remove crop
        val remover = BlockRemover()
        val result = remover.remove(testProjectDir, "test_crop", "testremove", RemovalOptions(force = true))
    ...
    fun `remove stairs block deletes all variant models`() {
        ...
        // Remove stairs
        val remover = BlockRemover()
        val result = remover.remove(testProjectDir, "test_stairs", "testremove", RemovalOptions(force = true))

        assertTrue(result.success)
        assertFalse(File(testProjectDir, "shared/common/src/main/java/com/testremove/blocks/TestStairs.java").exists())
        assertFalse(File(testProjectDir, "versions/shared/v1/assets/testremove/blockstates/test_stairs.json").exists())
    ...
    fun `create then remove item workflow`() {
        ...
        val itemFile = File(testProjectDir, "shared/common/src/main/java/com/testremove/items/WorkflowItem.java")
        assertTrue(itemFile.exists())

        // Remove
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "workflow_item", "testremove", RemovalOptions(force = true))
    ...
    fun `create multiple items then remove one`() {
        ...
        // Remove only item_b
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "item_b", "testremove", RemovalOptions(force = true))

        assertTrue(result.success)

        // Verify item_a and item_c still exist
        assertTrue(File(testProjectDir, "shared/common/src/main/java/com/testremove/items/ItemA.java").exists())
        assertFalse(File(testProjectDir, "shared/common/src/main/java/com/testremove/items/ItemB.java").exists())
        assertTrue(File(testProjectDir, "shared/common/src/main/java/com/testremove/items/ItemC.java").exists())
    ...
    fun `backup is created before deletion`() {
        ...
        // Remove with backup
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "backup_item", "testremove", RemovalOptions(force = true, createBackup = true))
    ...
    fun `remove item without recipe works`() {
        ...
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "no_recipe_item", "testremove", RemovalOptions(force = true))
    ...
    fun `remove block without loot table works`() {
        ...
        val remover = BlockRemover()
        val result = remover.remove(testProjectDir, "no_loot_block", "testremove", RemovalOptions(force = true))
    ...
    fun `file count in result is accurate`() {
        ...
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "count_item", "testremove", RemovalOptions(force = true))
    ...
    fun `removal result contains warnings for dependencies`() {
        ...
        // Create dependency
        val recipeFile = File(testProjectDir, "versions/shared/v1/data/testremove/recipe/uses_warn_item.json")
        FileUtil.writeText(recipeFile, """
            {
              "type": "minecraft:crafting_shapeless",
        ...
        val remover = ItemRemover()
        val result = remover.remove(testProjectDir, "warn_item", "testremove", RemovalOptions(force = false))
    ...
    fun `remove many items in sequence`() {
        ...
        var successCount = 0
        for (i in 1..10) {
            val result = remover.remove(testProjectDir, "bulk_item_$i", "testremove", RemovalOptions(force = true))
    ...
    fun `remove different component types in same project`() {
        ...
        val itemResult = itemRemover.remove(testProjectDir, "mixed_item", "testremove", RemovalOptions(force = true))
        val blockResult = blockRemover.remove(testProjectDir, "mixed_block", "testremove", RemovalOptions(force = true))
    ...
    fun `dependency analyzer finds all reference types`() {
        ...
        // Recipe dependency
        val recipeFile = File(testProjectDir, "versions/shared/v1/data/testremove/recipe/uses_dep_test.json")
        FileUtil.writeText(recipeFile, """{"ingredients": [{"item": "testremove:dep_test_item"}]}""")

        // Tag dependency
        val tagFile = File(testProjectDir, "versions/shared/v1/data/testremove/tags/items/test_tags.json")
        FileUtil.writeText(tagFile, """{"values": ["testremove:dep_test_item"]}""")

        // Loot table dependency
        val lootFile = File(testProjectDir, "versions/shared/v1/data/testremove/loot_table/blocks/test_block.json")
        FileUtil.writeText(lootFile, """{"pools": [{"entries": [{"name": "testremove:dep_test_item"}]}]}""")

        val dependencies = DependencyAnalyzer.findAllDependencies(testProjectDir, "dep_test_item", "testremove")
